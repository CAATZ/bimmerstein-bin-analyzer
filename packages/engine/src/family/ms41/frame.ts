 ...
 * not in config.ts.
 *
 * Flash address-line inversion applies in every bank: file = cpu ^ 0x4000.
 * This conversion is for flash-backed addresses, not RAM or SFRs.
 * Cal lives at CPU 0x10000+SA (DPP0=4), so fo(SA) = (0x10000+SA) ^ 0x4000.
 * Disassembly function labels may be file offsets already; CALLS operands
 * are CPU addresses (e.g. 0x027b1c → 0x023b1c, 0x034ba6 → 0x030ba6).
 */
...
export const cpuToFile = (cpu: number): number => cpu ^ MS41_BANK_XOR;
