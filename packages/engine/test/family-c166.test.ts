...
describe('C166_OPCODE_LEN', () => {
  ...
  it('holds only 0/2/4 and covers 198 opcodes', () => {
    expect(C166_OPCODE_LEN).toHaveLength(256);
    expect(C166_OPCODE_LEN.every((l) => l === 0 || l === 2 || l === 4)).toBe(true);
    expect(C166_OPCODE_LEN.filter((l) => l !== 0)).toHaveLength(198);
  });
});
...
describe('classifyReaderWidth', () => {
  /** Plant a callee body at file offset 0x1000 of a bank-0 buffer (cpuToFile(0x1000) = 0x1000). */
  function body(bytes: number[]): Uint8Array {
    const buf = new Uint8Array(0x2000);
    buf.set(bytes, 0x1000);
    return buf;
  }
